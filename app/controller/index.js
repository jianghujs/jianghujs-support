...
class IndexController extends Controller {
  ...
  async setting() {
    ...
    // 默认写入 userinfo
    if (!userInfo) {
      await jianghuKnex(tableEnum.user_info).insert({userId});
    }
    
    await this.ctx.render(`page/setting.html`, configs);
  }
  ...
  async collect() {
    ...
    const { userId } = this.ctx.userInfo;
    const { tab } = this.ctx.params;
    const configs = await this.userInfoAndConstant();
    
    const userInfo = userId ? await jianghuKnex(tableEnum.view01_user_info).where({userId}).first() : {};
    const userFavoriteCategoryIdList = _.compact((userInfo.userFavoriteCategoryList || '').split(','));
    const userFavoriteUserIdList = _.compact((userInfo.userFavoriteUserIdList || '').split(','));
    configs.articleList = await jianghuKnex(tableEnum.view01_article).where({userId}).select();
    configs.categoryList = await jianghuKnex(tableEnum.category).whereIn('categoryId', userFavoriteCategoryIdList).select();
    configs.userList = await jianghuKnex(tableEnum.view01_user_info).whereIn('userId', userFavoriteUserIdList).select();
    configs.tab = tab;
    
    await this.ctx.render(`page/collect.html`, configs);
  }
  ...
  async tag() {
    ...
    const { tag  } = this.ctx.params;
    const { p = 1  } = this.ctx.query;
    const pageSize = 40;
    const configs = await this.userInfoAndConstant();

    
    const knexQuery = jianghuKnex(tableEnum.article).whereRaw('articlePublishTime < ?', [ dayjs().format() ]).whereRaw(`FIND_IN_SET("${tag}", articleTagList)`);
    const queryRes = await this.queryArticle(knexQuery, p, 40);
      if (!queryRes) return;
    Object.assign(configs, queryRes);
    await this.ctx.render(`page/tag.html`, configs);
  }
  ...
  async write() {
    ...
    const { node: categoryId, title = '', content = '' } = this.ctx.query;
    const configs = await this.userInfoAndConstant();

    // 分类详情
    configs.categoryList = await jianghuKnex(tableEnum.category).select(['categoryId','categoryName']);
    configs.categoryList.forEach(e => {
    ...
    await this.ctx.render(`page/write.html`, configs);
  }
